...
def is_profitable(A1, B1, A2, B2, WA1, WB1, WA2, WB2, F1, F2):
	"""Checks if derivative at 0 is positive. Positive derivative means profitable opportunity."""
	return (WA1/WB1 * (WB2/WA2)**2 * (1-F1)*(1-F2)*A2*B1) / (B2*A1) - 1 >= 0
...
def function_generator(A1, B1, A2, B2, WA1, WB1, WA2, WB2, F1, F2):
	...
	f = lambda m: j*(k-i*m**-w1)**-(w2+1) * m**-(w1+1) - 1
	m = lambda x: q1*x+A1
	x = lambda m: (m-A1)/q1
...
def main(data):
	...
	if profitable: 
		...
		result = riddlers_method(f, a, b, MAX_ITER, ACCURACY)
...
if __name__=="__main__":
	...
	data = 	{"reserveOfToken1InPool1": 19259.5469, 
		     "reserveOfToken2InPool1": 21042.0552, 
		     "reserveOfToken1InPool2": 1415, 
		     "reserveOfToken2InPool2": 2333, 
		     "weightOfToken1Pool1": 0.65, 
		     "weightOfToken2Pool1": 0.35, 
