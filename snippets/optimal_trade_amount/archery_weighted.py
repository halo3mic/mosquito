...
def weighted_optimal_amount(A1, B1, A2, B2, WA1, WB1, WA2, WB2, F1, F2):
	"""
	...
	m = lambda x: (q2*A1**w1*B1) / (q1*x + A1)**w1
	x = lambda m: ((q2*A1**w1*B1/m)**(1/w1)-A1)/q1
...
if __name__=="__main__":
	...
	data = 	{"reserveOfToken1InPool1": 2500000, 
		     "reserveOfToken2InPool1": 9500, 
		     "reserveOfToken1InPool2": 2644755, 
		     "reserveOfToken2InPool2": 9385, 
		     "weightOfToken1Pool1": 0.7, 
		     "weightOfToken2Pool1": 0.3, 
	...
	f, m, m_inv, profit = weighted_optimal_amount(data["reserveOfToken1InPool1"], 
	...
	result = riddlers_method(f, a, b, max_iter=MAX_ITER, accuracy=MAX_ITER)
	# Result is given in terms of m(not x) and needs to be converted to x
	in_amount = m_inv(result)

	pprint(data)
	print(in_amount)
	print(profit(in_amount))



	# Add safe guard
	# Test edge cases
	# Is the second root always negative?
